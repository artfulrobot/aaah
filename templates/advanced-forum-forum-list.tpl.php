...
<?php foreach ($tables as $table_id => $table): ?>
  ...
  <div class="forum-table-wrap">
    ...
    <div id="forum-table-<?php print $table_info->tid; ?>">
      <div class="forum-table forum-table-forums">
        <div id="forum-table-<?php print $table_info->tid; ?>-content">
          <?php foreach ($table['items'] as $item_id => $item): ?>
            <?php if ($item->is_container): ?>
              <article id="subcontainer-<?php print $item_id; ?>" class="forum-row <?php print $item->zebra; ?> container-<?php print $item_id; ?>-child">
              <?php elseif ($item_id == TID_VOLUNTEERS_FORUM && !dcn_allowed_volunteers_forum()):
                // Skip the volunteers forum if this is not a volunteer.
                continue;
              ?>
              <?php else: ?>
              <article id="forum-<?php print $item_id; ?>" class="forum-row <?php print $item->zebra; ?> container-<?php print $item_id; ?>-child">
              <?php endif; ?>


              <div class="forum-details">
                <div class="forum-name-icons-wrapper">
                  <h1 class="forum-name">
              ...
              <?php if (!$item->is_container): ?>
                <div class="forum-number-topics">
                  <div class="forum-number-topics-total"><?php print $item->total_topics ?> topics
                    <?php if ($item->new_topics): ?>
                      <div class="forum-number-new-topics">
                        <a href="<?php print $item->new_topics_link; ?>"><?php print $item->new_topics_text; ?> new</a>
                      </div>
                    <?php endif; ?>
                  </div>
                </div>
                ...
                <div class="forum-number-posts">
                  <?php print $item->total_posts ?> posts

                  <?php if ($item->new_posts): ?>
                    <br />
                    <a href="<?php print $item->new_posts_link; ?>"><?php print $item->new_posts_text; ?> new</a>
                  <?php endif; ?>
                </div>
